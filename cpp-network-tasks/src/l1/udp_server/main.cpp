...
int main(int argc, char const *argv[])
{
    ...
    char client_hostname_buf[NI_MAXHOST];

    std::cout << "Running echo server...\n" << std::endl;
    char client_address_buf[INET_ADDRSTRLEN];

    std::string command_exit = "exit";
    bool is_exit = false;
    ...
    while (!is_exit)
    {
        ...
        if (recv_len > 0)
        {
            getnameinfo(reinterpret_cast<sockaddr*>(&client_address), client_address_len, client_hostname_buf, sizeof(client_hostname_buf), nullptr, 0, NI_NAMEREQD);
            buffer[recv_len] = '\0';

            if(!command_exit.compare(0, command_exit.size(), buffer, 0, recv_len - 1))
            {
                is_exit = true;
            }
